...
impl InstructionHandler {
    ...
    fn split_params(
        ...
        (Map<String, Value>, BTreeMap<String, String>, Option<Map<String, Value>>),
        InstructionError,
    > {
        let arg_names: HashSet<&str> = self.args.iter().map(|arg| arg.name.as_str()).collect();
        let account_names: HashSet<&str> =
            self.accounts.iter().map(|account| account.name.as_str()).collect();
            ...
            if arg_names.contains(key.as_str()) {
                ...
                let Value::Object(map) = value else {
                    return Err(InstructionError::InvalidValue {
                        ...
                        message: "Parameter \"resolve\" must be an object when provided".to_string(),
                    });
                };
                resolve = Some(map);
                ...
                return Err(InstructionError::UnknownParameter {
                    ...
                    accounts: self.accounts.iter().map(|account| account.name.clone()).collect(),
                });
            }
        }
...
mod tests {
    ...
    fn make_handler() -> InstructionHandler {
        InstructionHandler {
            ...
            args: vec![ArgSchema { name: "amount".to_string(), ty: ArgType::U64 }],
    ...
    fn payer_options() -> BuildOptions {
        BuildOptions { payer: Some(WSOL_MINT.to_string()), ..BuildOptions::default() }
    }

    fn state_pda(authority: &str) -> Pubkey {
        let seeds = vec![b"state".to_vec(), bs58::decode(authority).into_vec().unwrap()];
    ...
    fn splits_merged_params_into_args_and_account_overrides() {
        let built = make_handler()
            .build_with(json!({ "amount": 100, "mint": SYSTEM_PROGRAM }), &payer_options())
            .unwrap();
    ...
    fn options_account_overrides_win_over_params() {
        ...
        let built = make_handler()
            ...
            .unwrap();
        assert_eq!(built.accounts[1].pubkey, Pubkey::from_str(TOKEN_PROGRAM).unwrap());
    }
    ...
    fn rejects_non_string_account_params() {
        let err = make_handler()
            ...
            .unwrap_err();
        assert!(err.to_string().contains("not a known argument"), "unexpected error: {err}");
    }
    ...
    fn accepts_helper_only_resolve_inputs_for_pda_derivation() {
        ...
        .0;
        let pubkeys: Vec<Pubkey> = built.accounts.iter().map(|a| a.pubkey).collect();
        assert_eq!(pubkeys, vec![Pubkey::from_str(WSOL_MINT).unwrap(), expected]);
    ...
    fn rejects_non_object_resolve_inputs() {
        for resolve in [json!(1), json!(null), json!([1])] {
            let err = make_handler()
                ...
                .unwrap_err();
            assert!(err.to_string().contains("resolve"), "unexpected error: {err}");
        }
    }
    ...
    fn reports_missing_required_accounts() {
        let err = make_handler().build_with(json!({ "amount": 1 }), &payer_options()).unwrap_err();
        assert_eq!(err, InstructionError::MissingAccounts(vec!["mint".to_string()]));
    ...
    fn rejects_non_object_params() {
        ...
        assert!(err.to_string().contains("expected a JSON object"), "unexpected error: {err}");
    }
    ...
    fn looks_up_idl_errors_by_code() {
        ...
        assert_eq!(handler.error_for_code(6000).map(|e| e.name.as_str()), Some("Boom"));
