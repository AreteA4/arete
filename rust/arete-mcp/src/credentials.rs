    ...
    if is_hosted_websocket_url(url) {
        ...
        Err(anyhow!(
            "no Arete api key found for hosted stack `{url}`. \
             ...
             Fix: run `a4 auth login`, or set `{ENV_VAR_API_KEY}=a4-sk_...` (or legacy `hsk_...`) in your MCP \
...
mod tests {
    ...
    fn explicit_argument_wins_over_env_and_file() {
        let env = TestEnv::default()
            .with_var(ENV_VAR_API_KEY, "a4-sk_from_env")
            .with_credentials("api_key = \"a4-sk_from_file\"");
        let r = resolve_with(
            &env,
            Some("a4-sk_explicit".into()),
            "wss://foo.stack.arete.run",
        )
        .unwrap();
        assert_eq!(r.source, KeySource::Explicit);
        assert_eq!(r.key.as_deref(), Some("a4-sk_explicit"));
    }
    ...
    fn env_var_wins_over_file() {
        ...
        assert_eq!(r.key.as_deref(), Some("a4-sk_from_env"));
    }

    #[test]
    fn file_used_when_nothing_else_available() {
        let env = TestEnv::default().with_credentials("api_key = \"a4-sk_from_file\"");
        let r = resolve_with(&env, None, "wss://foo.stack.arete.run").unwrap();
        assert_eq!(r.source, KeySource::CredentialsFile);
        assert_eq!(r.key.as_deref(), Some("a4-sk_from_file"));
    }

    #[test]
    fn whitespace_only_explicit_falls_through_to_env() {
        let env = TestEnv::default().with_var(ENV_VAR_API_KEY, "a4-sk_from_env");
    ...
    fn parses_legacy_top_level_api_key() {
        assert_eq!(
            parse_credentials_content("api_key = \"a4-sk_legacy\"\n", DEFAULT_API_URL).as_deref(),
            Some("a4-sk_legacy")
        );
    }
    ...
    fn new_format_respects_api_url_selector() {
        let content = "[keys]\n\
                       \"https://api.arete.run\" = \"a4-sk_prod\"\n\
                       \"http://localhost:3000\" = \"a4-sk_local\"\n";
        assert_eq!(
    ...
    fn env_api_url_overrides_default_lookup() {
        ...
        let r = resolve_with(&env, None, "wss://foo.stack.arete.run").unwrap();
        assert_eq!(r.source, KeySource::CredentialsFile);
        assert_eq!(r.key.as_deref(), Some("a4-sk_local"));
    }
