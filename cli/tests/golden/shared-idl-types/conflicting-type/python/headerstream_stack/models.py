...
__all__ = [
    ...
    "BetaHeader",
    "beta_header_from_wire",
    "beta_header_patch_from_wire",
    "BetaVaultId",
...
def beta_header_from_wire(value: Any) -> BetaHeader:
    """Converts a wire payload into :class:`BetaHeader`.

    Raises ``ValueError`` when a required field is absent."""
    data = _mapping(value, "BetaHeader")
    return BetaHeader(
        version=_to_int(_require(data, "version", "BetaHeader")),
        owner=_require(data, "owner", "BetaHeader"),
        flags=_to_int(_require(data, "flags", "BetaHeader")),
    )


def beta_header_patch_from_wire(value: Any) -> Dict[str, Any]:
    """Converts a partial `BetaHeader` patch; only present keys appear."""
    data = _mapping(value, "BetaHeader patch")
    out: Dict[str, Any] = {}
    if "version" in data:
        out["version"] = _to_int(data["version"])
    if "owner" in data:
...
class BetaVaultState:
    ...
    header: Optional[BetaHeader] = None
...
def beta_vault_state_from_wire(value: Any) -> BetaVaultState:
    ...
    return BetaVaultState(
        header=_convert(data.get("header"), beta_header_from_wire),
    )
...
def beta_vault_state_patch_from_wire(value: Any) -> Dict[str, Any]:
    ...
    out: Dict[str, Any] = {}
    if "header" in data:
        out["header"] = _convert(data["header"], beta_header_from_wire)
    return out
