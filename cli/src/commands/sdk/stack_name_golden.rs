...
/// Generated Rust and Python SDKs pin the `arete-sdk` release they were
/// generated by. Goldens record `{{ARETE_VERSION}}` there instead, so a release
/// bump never invalidates them. Those two SDKs' `sdk-manifest.json` also carry
/// `sdkOutputTreeHash`, a hash over the files that contain the pin, so it is
/// recorded as `{{SDK_OUTPUT_TREE_HASH}}`; the files it covers are still compared
/// byte for byte. TypeScript SDKs pin no release and are compared unchanged.
