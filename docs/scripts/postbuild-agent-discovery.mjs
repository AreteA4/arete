...
const lines = [
  ...
  "## When to use Arete",
  "",
  "Use Arete when an agent needs to turn a Solana product question into exact, installable capabilities:",
  "",
  "- Discover programs or hosted live views from intent (`a4 explore catalog --query \"<intent>\" --json`) instead of guessing program IDs.",
  "- Install typed TypeScript, React, Rust, or Python SDKs bound to a catalog descriptor (`a4 install`).",
  "- Query or subscribe to live views through generated clients, `a4 stream`, or the stream MCP (`a4 mcp`).",
  "- Search and fetch this documentation through the docs MCP at https://docs.arete.run/mcp (`search_docs`, `fetch_page`) or by reading this file.",
  "",
  "Do not use Arete as a generic Solana RPC, a wallet, or a DEX. Do not infer instruction layouts from training data when a generated SDK exists. Hosted publication and deployment belong to `arete-deploy` / `a4 up`, not to scraping these docs.",
  "",
  "How to call this site: send `Accept: text/markdown` to any docs URL, append `.md`, POST JSON-RPC to `/mcp`, or read the OpenAPI document at https://docs.arete.run/openapi.json. The Arete developer portal is https://docs.arete.run/developers.",
  "",
  "## Start here",
  "",
  ...promoted.map(bullet),
...
lines.push(
  ...
  `- [Agent skills index](${SITE}/.well-known/agent-skills): well-known v0.2 index of the five Arete product skills`,
  `- [Sitemap](${SITE}/sitemap.xml): XML sitemap of the HTML pages`,
  `- [OpenAPI spec](${SITE}/openapi.json): Arete documentation HTTP API for function calling`,
  `- [Arete developer portal](${SITE}/developers): API keys, quickstart, MCP, OpenAPI`,
  `- [About Arete](${SITE}/about): what this product and docs site are`,
  `- [Contact](${SITE}/contact): GitHub issues, keys, and docs MCP`,
  `- [Privacy](${SITE}/privacy): docs-site analytics notice`,
  "",
