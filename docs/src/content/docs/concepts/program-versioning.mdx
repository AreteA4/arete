...
description: How Arete assigns independent semantic versions to content-addressed program releases.
---
...
An Arete version is separate from any version, release name, or upgrade number
maintained by the protocol itself. For example, `spl-token@1.0.2` means version
`1.0.2` of Arete's `spl-token` package. It does not rename or replace the
upstream SPL Token program's own release history.
...
## What an Arete version does not claim

An Arete version does not assert that:

- the protocol uses the same version number;
- the protocol assigned semantic meaning to an on-chain upgrade;
- Arete owns or governs the program;
- the program has been audited or is safe to use; or
- a higher version is preferable for every application.

It tells you how Arete classified and packaged an observed program interface
and the exact artifacts behind it. Use protocol-owned sources for upstream
release policy, governance, and security information.
