...
Explore and analyze Solana IDL files directly from the command line. An IDL (Interface Definition Language file) is a JSON description of a program: the instructions it accepts, the accounts it stores, and the types they use.
...
- **Fuzzy matching**: Lookups for instructions, accounts, types, and events are case-insensitive. If you make a typo, the CLI suggests the closest match.
...
| [`event`](#a4-idl-event)                 | Detail view of a specific event          |
...
# Inspect resolved event payloads as JSON
a4 idl events subscriptions.json --json
```

`--json` returns enriched event objects with resolved payload fields. For IDLs that omit inline event fields, the CLI falls back to the event's explicit backing type or a same-name struct in `types[]`.
...
Show the full definition of a specific event, including its discriminator, docs, resolved payload fields, and the source used to resolve those fields.

```bash
# Detail view for an event
a4 idl event subscriptions.json SubscriptionTransferEvent

# Machine-readable output
a4 idl event subscriptions.json SubscriptionTransferEvent --json
```
...
Perform a fuzzy search across instructions, accounts, types, errors, events, event payload fields, and constants.
...
# Search for event payload fields directly
a4 idl search subscriptions.json periodStartTs --json
```

Event-field matches include a `path` like `SubscriptionTransferEvent.periodStartTs` and a `parent_name` pointing back to the owning event.
