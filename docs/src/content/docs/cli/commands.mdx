...
again. `--branch`, `--preview`, and `--json` deployments leave `arete.toml`
unchanged; pass their endpoints to the client instead, for example
`useArete(stack, { url, httpUrl })` in TypeScript.
