...
:::caution[Key casing is not uniform]
`explore_stacks` returns **snake_case** keys (`websocket_url`, `http_auth`) — it is the platform's oldest registry response and predates the convention. Every other discovery tool returns **camelCase**. Read the keys you actually receive rather than assuming one convention. The `a4 explore --json` CLI output is camelCase throughout, so it does not match `explore_stacks` field-for-field.
:::
