...
| Tool                   | Purpose                                                                                            |
| ---------------------- | -------------------------------------------------------------------------------------------------- |
| `explore_stacks`       | List stacks in the registry. The `websocketUrl` in each entry is what `connect` takes.             |
...
:::note[Discovery keys are camelCase]
Every discovery tool returns camelCase keys — `websocketUrl`, `httpAuth`, `stackName`,
`primaryKeys`, `rustType`, `installName`, `programSpecHash`, `stackManifestHash`. `a4 explore
--json` uses the same names, so CLI output and tool output transfer field-for-field.

The one exception is content-addressed payloads. `resolve_artifact` returns a camelCase
envelope (`artifactHash`, `artifactVersion`, `kind`, `payload`), but `payload` is the stored
artifact passed through byte-for-byte and may be snake_case inside — an embedded IDL's
`program_id` and `is_signer`, for example. The same applies to the `astPayload`,
`stackManifest` and `liveSpec` values inside `explore_stack`: the key is camelCase, the value
is frozen. Those bytes are hash input, so their casing cannot change.
:::
...
```
explore_stacks()                        → pick a stack, note its websocketUrl
