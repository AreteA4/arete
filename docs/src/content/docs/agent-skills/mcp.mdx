...
:::caution[Registry responses are snake_case — CLI output is camelCase]
Every discovery tool returns the registry's **snake_case** keys: `websocket_url`, `http_auth`, `stack_name`, `primary_keys`, `rust_type`. The `a4 explore --json` CLI output describes the same data in **camelCase** (`primaryKeys`, `rustType`). The two do not match field-for-field, so do not carry field names from one into the other — read the keys you actually receive.
:::
...
The discovery tools attach a resolved key **only when `ARETE_API_URL` names an Arete origin** (`arete.run`, any `*.arete.run` subdomain) or a loopback address for local development. Pointed anywhere else, they send unauthenticated requests rather than forwarding your key to that host — the endpoints they touch are public, so you still get the public result set.
