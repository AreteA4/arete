...
:::caution[Example availability]
This is a package-specific example retained for projects that already have
access to the ORE hosted stack. Before starting, run
`a4 explore catalog stack ore --json`. If the descriptor is unavailable or not
subscribe-ready for your identity, use the current
[Quickstart](/using-stacks/quickstart/) and choose a stack returned by the
catalog instead. Do not use the endpoint or generated names on this page with a
different package.
:::
