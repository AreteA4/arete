...
| `YELLOWSTONE_COMMITMENT`     | No                     | `processed` | Commitment level to ingest at; see below                  |
...
| `ARETE_JOURNAL_ENABLED`      | No                     | `false`     | Retains published events so append views replay by cursor |
| `ARETE_JOURNAL_MAX_RECORDS`  | No                     | `10000`     | Retained events per view                                  |
| `ARETE_JOURNAL_MAX_AGE_SECS` | No                     | `900`       | Age bound on retained events                              |
...
### Yellowstone commitment

`YELLOWSTONE_COMMITMENT` selects the level every Yellowstone subscription asks
for: the program stream, its reconnects and snapshot resumes, and the slot
subscription that drives scheduling and readiness. Accepted values are
`processed`, `confirmed` and `finalized` (case-insensitive). Unset means
`processed`, the level every existing deployment already ingests at. An empty or
unrecognised value stops the runtime at startup instead of falling back.

`processed` delivers first but can include slots the cluster later drops;
`finalized` delivers only rooted data, about 32 slots (roughly 13 seconds) behind
the tip.
`confirmed` sits between them. The runtime logs the level it resolved at startup and a live runtime reports it
as `yellowstone_commitment` on `/status`. Runtimes without live ingestion
(`http`, `program_reads`, the Solana gateway) report `null` there.

To set it per runtime rather than per process, use the builder:

```rust
use arete::server::Commitment;
...
A snapshot records the level it was taken at. A runtime never resumes from a
snapshot taken at a weaker level than its own, because that state may hold
slots the stronger level never delivers; it cold-starts instead. A stronger
snapshot resumes normally at a weaker level.

Transaction routes and chain reads keep their own `confirmed` RPC commitment;
...
| `/status`                | GET    | Health state, error count and the ingestion commitment                   |
...
| `live_fallback_attempts`    | `Option<u32>`      | `Some(3)`         | Short-lived connections before abandoning the resume checkpoint; `None` never abandons it |
...
| `.fail_closed()`                            | Never abandon the resume checkpoint |

### Losing the resume checkpoint

A runtime with no snapshot to replay resumes from the last slot it finished
processing. When the provider keeps dropping the connection shortly after
accepting it — usually because that slot has fallen outside its replay window
— the runtime gives up after `live_fallback_attempts` and subscribes live.

That trades data for availability: every slot between the checkpoint and the
live tip is lost. The loss is reported rather than silent — the event journal
records the discontinuity, so a consumer replaying across it is refused with
`replay-gap` instead of being served a hole as an unbroken stream.

A recorder should refuse the trade outright:

```rust
Server::builder()
    .reconnection_config(ReconnectionConfig::new().fail_closed())
    .start()
    .await?;
```

Ingestion then stalls on the checkpoint until the provider serves it, rather
than skipping ahead. Snapshot-restored replays never take the fallback in
either mode.
