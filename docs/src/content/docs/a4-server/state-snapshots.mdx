...
| `ARETE_SNAPSHOT_URL` | — | `file:///var/lib/arete/snapshots` or a plain path |
...
Only local filesystem URLs are supported in this build. A directory on a
persistent volume (for Kubernetes, a PVC) is all a self-hosted deployment
needs. An `object_store`-backed backend (S3/GCS/Azure) behind a
`snapshot-object-store` cargo feature is planned.
...
- In-flight async resolver requests — they re-queue on the next relevant
  event. Cached resolver results *are* preserved, honoring their original
