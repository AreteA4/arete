...
2. Dumps the VM entity tables, projection caches, and **resume watermark** —
   the highest slot of stream input the projector has applied — from that same
   processing cut. Scheduled resolver callbacks do not advance it: they are
   stamped with the live slot tip, which can be ahead of the parser.
