...
This page covers how an application sends transactions with Arete. The
transaction transport lets your app inspect, submit, and confirm transactions
through the transaction descriptor embedded in a generated stack or Program SDK.
It is independent of the live-view and Program Read endpoints.

One thing never changes: the wallet compiles and signs locally. Arete never
receives a private key and never signs on the user's behalf.
...
Prepare a generated operation, then inspect it without signing or prompting the wallet:
...
Generated program operations exposed by `useArete` have `.useMutation()` hooks.
A mutation prepares the operation before wallet approval, so program-specific
preparation can re-read authoritative accounts and reject stale input.
