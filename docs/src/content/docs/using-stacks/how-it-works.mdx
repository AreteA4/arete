...
const { data: tokens } = stack.views.Token.list.use();

// State - generated key object, with fields determined by the entity primary key
const { data: token } = stack.views.Token.state.use({ mint: mintAddress });
```
...
Package publication is a separate, manual operator action.
...
3. Review the selected views and run `a4 up <path.stack-manifest.json>`
4. The hosted control plane resolves immutable Program Releases and creates deployment bindings
...
Two similarly named options control different phases:

- React `autoConnect` defaults to `true` and controls only whether `AreteProvider` starts the initial connection.
- `autoReconnect` defaults to `true` in React and the core TypeScript SDK and controls only recovery after an established connection is lost.

Disabling `autoConnect` does not disable later recovery for a connection your app establishes. Disabling `autoReconnect` does not suppress the initial connection.
...
const { data: token } = stack.views.Token.state.use({ mint });
