...
Install the prebuilt, signed binary (no Rust toolchain needed):
...
The binary lands in `~/.local/bin/a4` and the installer prints `A4_BIN=<absolute path>`. If `a4` is not found in an already-open terminal, run `export PATH="$HOME/.local/bin:$PATH"`. Then check it:
...
No separate install step — the npm package downloads the same binary on first use:

```bash
npx @usearete/a4 create my-app
```

To install it explicitly (equivalent to `npm install -g @usearete/a4`):
...
for await (const update of session.stacks.ore.views.OreRound.latest.watch({ take: 1 })) {
  if (update.type === "upsert" || update.type === "patch") {
    console.log(`Round #${update.data.id.roundId}`);
...
Install Arete agent skills and MCP config so your AI can write correct code without you looking up docs:

```bash
a4 init -y          # skills for every detected agent + MCP servers + AGENTS.md
a4 doctor --json    # exit 0 = ready
```

Skills only, without the rest: `npx skills add AreteA4/skills`.
