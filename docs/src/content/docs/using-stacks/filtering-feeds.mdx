...
A view can hold far more data than any one screen needs. When you subscribe, you can tell the server exactly which slice to send: how many items, starting where, and matching which key. Asking for less saves bandwidth and keeps your app fast.
...
State subscriptions use a separate generated key object whose fields match the entity primary key.
...
<Tabs>
<TabItem label="Pagination — React">

```typescript
import { useArete } from "@usearete/react";
import { TOKEN_STACK } from "./generated/token-stack";
...
<TabItem label="Pagination — TypeScript">
...
const session = await createSession({
  stacks: { token: TOKEN_STACK },
});
const a4 = session.stacks.token;
...
<Tabs>
<TabItem label="State view — React">

```typescript
const { data: token } = a4.views.Token.state.use({ mint: tokenAddress });
```

</TabItem>
<TabItem label="State view — TypeScript">
...
<Tabs>
<TabItem label="Custom views — React">
...
function LatestRounds() {
  ...
  const { data: round } = a4.views.OreRound.state.use({ roundId });

  // Custom history view - sorted by round ID descending
  const { data: latestRounds } = a4.views.OreRound.latest.use();

  return <div>{latestRounds?.map(r => <Round key={r.id.roundId?.toString()} round={r} />)}</div>;
}
```

`OreRound/latest` is appropriate for history. Follow `OreBoard/state` to keyed `OreRound/state` when selecting ORE's active round.

</TabItem>
<TabItem label="Custom views — TypeScript">
...
<TabItem label="Custom views — Rust">
...
for await (const token of a4.views.Token.topByVolume.use()) {
  console.log(token.id.mint, token.metrics.totalVolume);
}
```
...
<Tabs>
<TabItem label="Client-side filter — React">

Use standard JavaScript to derive a filtered list from the streamed query result:
...
function HighVolumeTokens() {
  ...
  const stream = a4.views.Token.list.use({ take: 100 });
  const tokens = (stream.data ?? [])
    .filter(token => token.metrics.volume >= 10000 && token.metrics.price <= 100)
    .slice(0, 20);
...
<TabItem label="Client-side filter — TypeScript">
...
for await (const round of a4.views.OreRound.latest.use()) {
  ...
  console.log("High-value round:", round.id.roundId);
}
```

</TabItem>
<TabItem label="Client-side filter — Rust">
