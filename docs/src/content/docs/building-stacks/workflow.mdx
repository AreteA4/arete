...
:::note[This is the advanced path]
Everything under **Create Live Data** is for building your own live views in
Rust. Most projects never need it. If you haven't yet searched the catalog or
explored existing views with your agent, start with
[Explore On-chain with MCP](/agent-skills/explore-on-chain/).
:::

Most applications should begin with published Program SDKs and existing live
views. Turn to custom authoring only when the catalog doesn't offer the
app-shaped view you need.

Search the registry and open the exact descriptor before starting custom work:
...
If a published package already provides the required capabilities, install it
instead. The advanced Rust path below assumes you have the exact local IDL for
the program. Inspect that file with `a4 idl summary`, `a4 idl search`, and the
[IDL CLI reference](/cli/idl/) before mapping it.

For an uncataloged or private program, supply the canonical IDL from the
program's own build or release artifacts, upstream repository, or published SDK
package. An on-chain Anchor IDL is suitable only when the program intentionally
publishes one and you verify its program address and provenance. If you cannot
obtain a trustworthy IDL, ask the program owner rather than reconstructing one
from explorer output.

This page documents the advanced **Rust authoring path**: define a live model,
compile portable artifacts, deploy an exact StackManifest, and install the
resulting SDK.
