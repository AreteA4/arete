...
When you build your own live view, you describe it in a small Rust-based language (a DSL, or domain-specific language). You say how on-chain Solana data should be transformed, aggregated, and streamed to your application. You don't write the pipeline that does the work. You declare the final state you want, and Arete handles the rest.

---

## Why Declarative?

Building a data pipeline for Solana usually means parsing accounts by hand, handling a tangle of events, and keeping state in sync yourself. That is the imperative approach: you spell out every step. Arete's model is declarative: you describe the result.
...
An entity is owned by the program it reads. An entity that reads from more than one program must declare its owner, or the stack fails to compile:
