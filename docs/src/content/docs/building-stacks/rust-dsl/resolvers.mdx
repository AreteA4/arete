...
for await (const round of a4.views.OreRound.latest.use()) {
  ...
  console.log(round.oreMetadata?.logoUri); // "https://..."
}
```
...
| `logoUri`  | `string \| null` | URL to the token's logo image in generated TypeScript |
...
On the client, `motherlodeUi` arrives as a ready-to-display number:

```typescript
for await (const round of a4.views.OreRound.latest.use()) {
  console.log(round.motherlodeUi); // 1.5 (human-readable ORE amount)
}
```
