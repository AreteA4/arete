...
:::note[Legacy input]
Composite `.stack.json` is an input-only compatibility format through **August
31, 2026**. New builds consume the explicit macro outputs. Use
`a4 live build <legacy.stack.json>` only when normalizing an older input.
:::
