...
| `program` | `string` | No\*     | The program that owns the entity, by IDL program name (`"ore"` for `ore_sdk`). Must be a program the entity reads. |

_\* Required when the entity reads from more than one program, for example `#[entity(name = "OreRound", program = "ore")]`. Without it the stack fails to compile, because the owning program would be ambiguous. A single-program entity is owned by the program it reads._
...
| `fields`     | `array`    | Yes      | List of fields to capture. Use `accounts::name` for instruction accounts and `args::name` (or `data::name`) for instruction arguments. Event sources expose payload fields only. |
| `strategy`   | `Strategy` | No       | Update strategy (default: `SetOnce`).                                                                                                                                            |
...
| `field`     | `field`           | No       | Field to aggregate. Use `accounts::name` or `args::name` for instructions, or payload fields for events. If omitted, performs `Count`. |
