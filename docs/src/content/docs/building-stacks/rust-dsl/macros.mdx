...
Arete uses Rust procedural macros to define data pipelines declaratively. The
macros generate local runtime code and write separate ProgramSpec, LiveSpec, and
StackManifest artifacts directly. This keeps portable program identity, live
projection behavior, client composition, and hosted deployment bindings from
becoming one coupled file.
...
With entities, `#[arete]` generates:

```text
.arete/<program>.program-spec.json
.arete/<ModuleName>.live-spec.json
.arete/<ModuleName>.stack-manifest.json
```

The generated `spec()` combines projection bytecode, views, parsers, and local
program account readers for the OSS code-first server path. Its program runtime
definitions use automatic local release fingerprints; they do not require a
hosted Program Release or decoder binding.

With only IDLs and no entities, the same macro generates ProgramSpecs, a
zero-live StackManifest, and a program-only `spec()`:

```rust
#[arete(idl = ["idl/token.json"])]
pub mod token_program {}

let spec = token_program::spec();
```

The program-only spec has generated account readers and no live projection
work. This is useful with `Server::builder().spec(spec).program_reads()`.
...
Maps a field from a Solana account, instruction, or IDL event directly to an entity field.
...
The path prefix is derived from the IDL's program name. Common forms are:

- `my_program_sdk::accounts::AccountName::field_name`
- `my_program_sdk::instructions::InstructionName::field_name`
- `my_program_sdk::events::EventName::field_name`
- `my_program_sdk::events::EventName::accounts::account_name` (an account of the instruction that emitted the event, see [Event accounts](#event-accounts))

See [Stack Definitions](/building-stacks/stack-definitions) for the naming convention.

**Arguments:**

| Argument         | Type           | Required | Description                                                                                                                                                                                                 |
| ---------------- | -------------- | -------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `from`           | `path`         | Yes      | Source field path from an account, instruction, or IDL event (e.g., `AccountType::field_name`).                                                                                                             |
...
Captures multiple fields from an instruction or IDL event as a single structured record on the entity.
...
| Argument     | Type       | Required | Description                                                                                                                                                                                                                                              |
| ------------ | ---------- | -------- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `from`       | `path`     | Yes      | The source instruction or event type.                                                                                                                                                                                                                    |
| `fields`     | `array`    | Yes      | List of fields to capture. Use `accounts::name` for instruction accounts and `args::name` (or `data::name`) for instruction arguments. On an event source a bare name is a payload field and `accounts::name` is an account of the emitting instruction. |
| `strategy`   | `Strategy` | No       | Update strategy (default: `SetOnce`).                                                                                                                                                                                                                    |
...
An IDL event carries only its payload. `accounts::name` selects an account of the instruction that emitted the event, by its IDL account name, so an event mapping can record the accounts the payload leaves out. PumpSwap's `BuyEvent` and `SellEvent`, for example, omit the pool's mints, which the `buy` and `sell` instructions pass as accounts:
...
#[event(from = pump_amm_sdk::events::BuyEvent,
        fields = [base_amount_out, quote_amount_in, accounts::base_mint, accounts::quote_mint],
        lookup_by = pool, strategy = LastWrite)]
pub last_buy: Option<serde_json::Value>,
```

- The accounts come from the instruction occurrence that emitted that event. For `emit_cpi!` events, which arrive as their own inner instruction, that is the instruction that made the self-CPI. For `emit!` events, it is the instruction whose own logs hold the `Program data:` line.
- A bare name stays a payload field, even when an account has the same name (`pool`, `user`). One capture cannot hold both `pool` and `accounts::pool`.
- The name must be an account of some instruction in the IDL. The IDL does not say which instruction emits an event, so this check cannot be narrower.
- If the emitting instruction was not decoded (for example, its discriminator is not in your IDL), the account fields stay unset and the runtime logs a warning. They are never filled from another instruction.
- `accounts::` works in `#[map]` paths, `fields`, `field`, `lookup_by`, `stop_lookup_by`, and as an `accounts.name` path in conditions.
...
Defines a declarative aggregation from instructions or IDL events.
...
| Argument    | Type              | Required | Description                                                                                                                                                                                              |
| ----------- | ----------------- | -------- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `from`      | `path` \| `array` | Yes      | Instruction(s) or event(s) to aggregate from.                                                                                                                                                            |
| `field`     | `field`           | No       | Field to aggregate. Use `accounts::name` or `args::name` for instructions. For events, use a payload field or `accounts::name` for an account of the emitting instruction. If omitted, performs `Count`. |
...
**One instruction, several entity instances.** Each aggregate is routed by its own `lookup_by`, even when another field routes the same instruction differently. For example, a split instruction can count on both the source and the child position:

```rust
#[aggregate(from = SplitPosition, strategy = Count, lookup_by = accounts::first_position)]
pub split_source_count: Option<u64>,

#[aggregate(from = SplitPosition, strategy = Count, lookup_by = accounts::second_position)]
pub split_child_count: Option<u64>,
```

The instruction updates the `first_position` instance and the `second_position` instance separately. A `#[map]` from that instruction follows the key that reads the same field (for example `SplitPosition::first_position` follows `lookup_by = accounts::first_position`). A mapping that reads none of the instruction's keys is a compile error, because the macro cannot tell which instance it belongs to.
...
Derives values from instruction metadata, instruction fields, or IDL event payload fields.
...
| Argument    | Type              | Required | Description                                                                |
| ----------- | ----------------- | -------- | -------------------------------------------------------------------------- |
| `from`      | `path` \| `array` | Yes      | Instruction(s) or event(s) to derive from.                                 |
| `field`     | `field`           | Yes      | Target field. Can be a special field or a regular instruction/event field. |
