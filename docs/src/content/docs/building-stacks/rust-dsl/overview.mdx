...
The macros compile your Rust code into optimized runtime bytecode and write
authoritative public artifacts directly:

- one ProgramSpec for each IDL program;
- a LiveSpec containing entities, mappings, handlers, and views; and
- a convenience StackManifest referencing those exact artifacts and selected
  views.

After `cargo build`, these files appear under `.arete/` as
`*.program-spec.json`, `*.live-spec.json`, and `*.stack-manifest.json`.
ProgramSpecs and StackManifests are portable and endpoint-free. A hosted
deployment and its live, Program Read, chain, and transaction bindings are
operational objects created later.

:::note[Legacy input]
A composite `.stack.json` may still be emitted for compiler compatibility, but
it is an input-only adapter through **August 31, 2026**. New SDK generation and
deployment use the explicit artifacts.
:::
...
### Program-only definitions

An IDL-only `#[arete]` module with no entities generates ProgramSpecs, a
...
That `spec()` contains generated account readers but no live views or projection
bytecode. It can serve release-pinned-style Program Reads locally without a
hosted registry. The local release fingerprint is derived automatically from
the ProgramSpec and generated decoder-engine contract; a self-hoster does not
create or select a managed Program Release.
