...
const round = await session.stacks.ore.views.OreRound.state.get(roundAddress);
...
for await (const round of session.stacks.ore.views.OreRound.state.use(roundAddress)) {
...
// State view
const round = session.stacks.ore.views.OreRound.state.getSync(roundAddress);
```
