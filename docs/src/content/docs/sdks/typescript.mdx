...
description: Framework-agnostic TypeScript client for generated Program SDKs, live views, chain reads, and transactions.
sidebar:
  order: 2
---

The `@usearete/sdk` package is the framework-agnostic client for generated
Program SDKs, live views, generic Solana reads, and transaction operations. It
uses `AsyncIterable`-based streaming and works in Node.js, browsers, Deno, and
Bun.

This is a reference page. If your agent writes the code, this is what it works
from, and it is where to look when you want to understand or review what the
agent wrote.
...
for await (const position of session.stacks.app.views.Position.list.use()) {
  console.log(position);
}
```

`MY_STACK` and `Position` are placeholders. Install a subscribe-ready stack and
use its generated import and view names.
...
Use `session.chain` for generic Solana reads and `session.execute(prepared)` for
any prepared instruction, transaction, or flow.
...
### Application Operations vs. Raw IDL

Prefer generated application operations when they are available. They accept
normal camelCase objects, derive routine addresses, normalize values such as UI
token amounts, and describe their cardinality through the namespace:
...
const positions = await session.stacks.app.views.Position.list.get({ take: 20 });

// Generic chain reads belong to the session console
const lamports = await session.chain.lamports(owner);
```
...
```typescript
// Check current state
console.log(session.stacks.app.connectionState);
// 'connected' | 'connecting' | 'disconnected' | 'reconnecting' | 'error'

// Listen for changes
const unsubscribe = session.stacks.app.onConnectionStateChange((state) => {
...
Executes a value returned by a generated instruction, transaction, or flow's
`.prepare(...)` method.
