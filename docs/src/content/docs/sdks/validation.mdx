...
description: Runtime Zod schemas generated alongside Arete TypeScript interfaces for wire and query validation.
sidebar:
  order: 4
---

TypeScript types protect you while you write code, but they can't check the data that actually arrives at runtime. So every Arete stack ships with [Zod](https://zod.dev) schemas alongside its TypeScript interfaces. These schemas give you runtime validation at two levels: **automatic frame validation** on the wire, and **per-query validation** in your application code.

---

## Generated Schemas

When you run `a4 sdk create --manifest <path.stack-manifest.json> --ts`, the CLI generates a Zod schema for every selected entity and sub-type. These mirror the TypeScript interfaces exactly:
