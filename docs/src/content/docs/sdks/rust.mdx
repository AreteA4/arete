...
| --------------- | --------------- | ----------------------------------------------------- |
| `.listen()`     | `T`             | Merged entity directly (filters out removals/deletes) |
| `.watch()`      | `Update<T>`     | Upsert/Patch/Remove/Delete events                     |
| `.watch_rich()` | `RichUpdate<T>` | Before/after diffs for tracking changes               |

### Simple Streaming with `.listen()`

The simplest API - emits merged entities directly, filtering out removals and deletes:
...
Stream all update types (upsert, patch, remove, delete):
...
while let Some(update) = stream.next().await {
    match update {
        ...
        Update::Remove { key } => {
            println!("Round left this query: {:?}", key);
        }
        Update::Delete { key } => {
...
while let Some(update) = stream.next().await {
    match update {
        ...
        RichUpdate::Removed { key, .. } => {
            println!("Left this query: {:?}", key);
        }
        RichUpdate::Deleted { key, .. } => {
            println!("Deleted: {:?}", key);
        }
    }
}
...
// Select a partition and cap only the initial snapshot
let mut stream = a4.views.ore_round.list()
    .watch()
    .partition("solana-mainnet")
    .with_snapshot_limit(100);

// Resume incrementally after an _seq cursor
let mut stream = a4.views.ore_round.list()
    .watch()
    .after("1234:000000000010");
```

Each builder defines exact protocol v2 query membership. Different windows and filters on the same view coexist independently. Equivalent normalized queries share one wire subscription, and dropping the final equivalent stream sends `unsubscribe`.

The SDK keeps a stable opaque subscription ID across reconnects. A complete authoritative snapshot atomically replaces only that query's membership; an `after` snapshot is incremental and merges without pruning.
...
| `.partition(value)`       | Exact `_partition` match     |
| `.with_snapshot(bool)`    | Enable the initial snapshot  |
| `.after(cursor)`          | Incremental `_seq` cursor    |
| `.with_snapshot_limit(n)` | Cap initial snapshot rows    |
...
Use the Arete CLI to generate a typed Rust SDK from an exact StackManifest. See [CLI Commands](/cli/commands/) for the full reference and [Configuration](/building-stacks/configuration/) for `arete.toml` options.
...
async fn main() -> anyhow::Result<()> {
    ...
    while let Some(update) = stream.next().await {
        ...
        match update {
            ...
            Update::Remove { key } => {
                println!("[#{:03}] Round left this query: {:?}", update_count, key);
            }
            Update::Delete { key } => {
                println!("[#{:03}] Deleted round: {:?}", update_count, key);
            }
        }
