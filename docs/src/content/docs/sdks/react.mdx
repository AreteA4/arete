---
title: React SDK
description: Provider-managed connections and React hooks for generated Arete Program SDKs and live views.
sidebar:
  order: 1
---
...
An application also needs a generated stack definition. Discover a
subscribe-ready package and install the target reported by its descriptor:
...
The generated entry point exports the stack definition, entity types, schemas,
program operations, addresses, reads, and package-specific helpers. Commit
generated output, but do not edit it by hand.
...
function Positions() {
  ...
  if (positions.status !== 'ready') return <p>Connecting...</p>;
...
export default function App() {
  return (
    <AreteProvider autoConnect stack={MY_STACK} auth={{ publishableKey }}>
      <Positions />
    </AreteProvider>
  );
}
...
`MY_STACK`, `Position`, and the available query parameters are placeholders.
Use the generated import, view names, keys, and fields for the package you
installed.

## Generated names and keys

Generated TypeScript uses language-native camelCase even when source schemas or
wire view paths use another convention. Use generated object paths rather than
spelling protocol view identifiers or translating fields yourself.
...
Read-only viewing does not require a wallet, but hosted browser access can still
require a publishable key as shown above.
...
- `programs`: generated account, instruction, operation, flow, and mutation hooks
...
Generated raw instructions and application operations expose `.useMutation()`:
