...
│                     YOUR APP                            │
│                                                          │
│   const { data } = arete.views.Token.list.use()         │
│                                                          │
└─────────────────────────────────────────────────────────┘
```
...
A **StackManifest** is the portable client composition of exact ProgramSpecs,
aliased LiveSpecs, and selected views. A deployment and its endpoint bindings
are separate. Generate the TypeScript definition with the CLI rather than
hand-writing view factories:

```bash
a4 sdk create --manifest .arete/MyTokens.stack-manifest.json --ts
```

```typescript
import { Arete } from '@usearete/sdk';
import { MY_TOKENS_STREAM_STACK } from './generated/my-tokens-stack';

const tokens = await Arete.connect(MY_TOKENS_STREAM_STACK);
```

Connected generated stacks expose typed views and, when defined, programs, reads, queries, addresses, constants, defaults, math, and flows.
...
1. Write a stream definition using Rust macros
2. Build generates ProgramSpec, LiveSpec, and StackManifest artifacts
3. Deploy the exact composition with `a4 up .arete/MyTokens.stack-manifest.json`
4. Cloud resolves immutable Program Releases and creates deployment bindings
...
const { data: tokens } = arete.views.Token.list.use()

// State - generated key object
const { data: token } = arete.views.Token.state.use({ mint: mintAddress })
```
...
const { data: token } = arete.views.Token.state.use({ mint })
console.log(token?.id.mint)             // string | undefined
console.log(token?.trading.totalVolume) // bigint | undefined
```
...
```tsx
const arete = useArete(MY_TOKENS_STREAM_STACK);
const { data: tokens } = arete.views.Token.list.use();
```
