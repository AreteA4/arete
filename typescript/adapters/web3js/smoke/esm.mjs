...
if (typeof createWalletAdapter !== 'function') {
  throw new Error('Packed ESM export is unavailable');
}
...
if (instruction.data[2] !== 3) {
  throw new Error('Packed ESM instruction conversion failed');
}
...
const wallet = createWalletAdapter({
  connection: {
    ...
    async sendRawTransaction() {
      sendCalls += 1;
      throw new Error('uncertain packed ESM send');
    },
    async getSignatureStatuses(_signatures, config) {
      statusCalls += 1;
      if (config?.searchTransactionHistory !== true) {
        throw new Error('Packed ESM status lookup did not search transaction history');
      }
...
if (outcome?.status !== 'submitted-unknown' || !outcome.signature) {
  throw new Error('Packed ESM uncertainty outcome or bs58 signature is unavailable');
}
if (sendCalls !== 1 || statusCalls !== 1) {
  throw new Error('Packed ESM uncertainty handling retried submission or status lookup');
}
