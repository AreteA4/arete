...
pub struct PythonStackConfig {
    ...
    /// Served version emitted as the `StackDef` `release`. Only StackManifest
...
fn generate_stack_init_py(
    ...
    let (stack_imports, release_kwarg) = match config.release.as_ref() {
        Some(release) => (
            "StackDef, StackEndpoints, StackRelease",
            format!(
                "\n    release=StackRelease(\n        stack_manifest_hash={},\n        live_alias={},\n    ),",
                py_string_literal(&release.stack_manifest_hash),
                py_string_literal(&release.live_alias),
            ),
        ),
        None => ("StackDef, StackEndpoints", String::new()),
    };
...
{gateway_imports}
from arete.stack import {stack_imports}
...
{ws_line}{http_line}
    ...
    views=views.VIEWS,{programs_kwargs}{gateway_kwarg}{release_kwarg}
)
